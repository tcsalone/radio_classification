...
capture_watchdog() {
  ...
  while kill -0 "$cap_pid" 2>/dev/null; do
    ...
    if [[ "$size" -gt "$last_size" ]]; then
      ...
      stalled_for=$((stalled_for + STALL_POLL))
      if [[ "$stalled_for" -ge "$STALL_TIMEOUT" ]]; then
        echo "capture_watchdog: STALL detected: $(basename "$newest") unchanged for ${stalled_for}s; killing capture to force restart" | tee -a "$SKIPS_LOG" >&2
        pkill -9 -x rtl_fm 2>/dev/null || true
        pkill -9 -f "radio_classifier capture chunks" 2>/dev/null || true
...
if [[ "$STALL_TIMEOUT" -gt 0 ]]; then
  capture_watchdog "$CAPTURE_PID" &
  WATCHDOG_PID="$!"
  trap 'kill "${WATCHDOG_PID:-}" 2>/dev/null || true' EXIT
  echo "capture stall-watchdog: pid=$WATCHDOG_PID stall_timeout=${STALL_TIMEOUT}s poll=${STALL_POLL}s"
fi
